...
if ($action === 'send') {
    ...
    try {
        ...
        $isConflict = ($res['code'] === 409 
            || ($rawCeisa['code'] ?? null) === 409 
            || (isset($rawCeisa['result']) && $rawCeisa['result'] === 'Data already exists.') 
            || (isset($rawCeisa['detail']) && stripos($rawCeisa['detail'], 'sudah pernah') !== false));
        $isRecorded = ($isOk || $isConflict);

        // 1. Simpan Audit Log ke ceisa_api_logs
        try {
            global $pdo_tpsonline;
            if ($pdo_tpsonline) {
                $logStatus = $isOk ? 'SUCCESS' : ($isConflict ? 'CONFLICT' : 'FAILED');
                $logMsg = $res['message'] ?? ($isOk ? 'Berhasil merekam tracking TPS' : ($isConflict ? 'Data sudah pernah dikirim sebelumnya (409 Conflict)' : 'Gagal'));
...
if ($action === 'get_container_timeline') {
    ...
    $dept = strtolower(trim((string)input('dept', 'tpp'))); // 'tpp' atau 'gudang'
    $result = getContainerTimelineData($noContRaw, $dept);
    jsonResp($result, ($result['success'] ?? false) ? 200 : 400);
}
